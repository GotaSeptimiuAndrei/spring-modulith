...
- **`common`**: A shared utility package containing global cross-cutting concerns, such as the `APIResponse` wrapper, `ErrorDTO` (which is included in the API response), and `JwtUtils` for security.
...
## 4. Architectural Verification and Automated Documentation
